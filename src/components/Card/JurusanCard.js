// components/Card/JurusanCard.js
import Link from 'next/link';
import Image from 'next/image';
import { motion } from 'framer-motion'; // Import motion

export default function JurusanCard({ id, slug, name, description, imageUrl, tagline }) {
  return (
    <motion.div
    ...
    >
      <Link href={`/jurusan/${slug}`} className="block">
        <div className="relative w-full h-48 sm:h-56 overflow-hidden rounded-t-xl">
          <Image
            src={imageUrl || '/images/default-jurusan.jpg'} // Fallback image
            alt={name}
            layout="fill"
            objectFit="cover"
