...
const AnnouncementsPage: FC = () => {
  ...
  const fetchAnnouncements = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (apiError?.message) {
        setError(`Gagal memuat data pengumuman. Detail: ${apiError.message}`);
  ...
  const confirmDelete = async (): Promise<void> => {
    ...
    try {
      await apiDelete<{ message: string }>(
        `/announcement?id=${confirmActionId}`
      );
      setNotification({
      ...
      const message =
        ...
        (e instanceof Error
          ? e.message
          : "Gagal menghapus pengumuman. Silakan coba lagi.");
      setNotification({
        message: `Gagal menghapus pengumuman: ${message}`,
        type: "error",
      });
    } finally {
      setIsConfirmModalOpen(false);
  ...
  const handleSaveAnnouncement = async (
    ...
    try {
      const payload = {
        ...
        date: new Date(newAnnouncement.publishDate).toISOString(),
        status: newAnnouncement.status === "Published" ? "PUBLISHED" : "DRAFT",
      };

      if (newAnnouncement.id) {
        await apiPut<AnnouncementsApiEnvelope>(
          `/announcement?id=${newAnnouncement.id}`,
          payload
        );
      } else {
        await apiPost<AnnouncementsApiEnvelope>("/announcement", payload);
      }
      setNotification({
        message: `Pengumuman berhasil ${
          newAnnouncement.id ? "diperbarui" : "ditambahkan"
        }!`,
        type: "success",
      });
      ...
      setNotification({
        message: `Gagal menyimpan pengumuman: ${errorMessage}`,
        type: "error",
      });
    }
  };
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 mt-1">
                    Kelola dan publikasikan pengumuman penting
                  </p>
                </div>
              </div>
              <button
                    ...
                    {announcements.length === 0 ? (
                          ...
                          <div className="flex flex-col items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-gray-500 mt-1">
                                Mulai dengan menambahkan pengumuman pertama Anda
                              </p>
                            </div>
                            <button
                              onClick={() => handleAddEdit()}
                      ...
                      announcements.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <span
                              className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${getStatusColorClass(
                                item.status
                              )}`}
                            >
                              <span
