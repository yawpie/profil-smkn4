...
const AnimatedCounter: React.FC<AnimatedCounterProps> = ({ from, to, duration = 1.5, suffix = '', prefix = '' }) => {
  ...
  useEffect(() => {
    if (isInView) {
      controls.start({
        // kita tidak pakai `count`, jadi kita trigger dummy animasi
        opacity: 1,
        transition: {
          duration,
          ease: 'easeOut',
...
const DashboardOverviewPage: React.FC = () => {
  ...
  const PIE_COLORS: string[] = ['#3B82F6', '#6366F1', '#8B5CF6', '#A855F7', '#EC4899', '#EF4444', '#10B981', '#06B6D4'];
  ...
  // Corrected Variants structure and eased type for 'ease'
  const sectionVariants: Variants = {
    ...
    visible: {
      ...
      transition: {
        duration: 0.8,
        ease: "easeOut", // 'easeOut' is a valid Easing type
        staggerChildren: 0.1 // staggerChildren is a direct property of the variant, not inside 'transition'
      }
    },
  };
  ...
  const cardItemVariants: Variants = {
    ...
    visible: {
      ...
      y: 0,
      transition: {
        type: "spring", // 'spring' is a valid AnimationType
        stiffness: 100,
        damping: 12
      }
    },
  ...
  return (
    <Layout setNotification={() => {}}> {/* Provide a dummy function or proper setNotification if Layout expects it */}
      <motion.div
        initial="hidden"
        animate="visible"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {dataCards.map((card, index) => (
              ...
              >
                ...
                <div className={`p-3 rounded-full ${card.iconBg} bg-opacity-70 backdrop-blur-sm relative z-10 flex-shrink-0`}>
                  {card.icon && <card.icon className="h-8 w-8" />}
                </div>
                <div className="relative z-10">
                  <h3 className="text-base font-semibold text-white opacity-90">{card.title}</h3>
      ...
      >
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                ...
                fill="#8884d8"
                dataKey="value"
                // Explicitly type name and percent in the label function
        ...
        <motion.div variants={cardItemVariants} className="bg-white rounded-xl shadow-lg p-8 lg:col-span-2">
